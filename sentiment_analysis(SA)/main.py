...
path3 = os.path.join(save_folder, "saved_expmodel.pt")
load_data = False
load_network = False
...
if os.path.exists(path2) & os.path.exists(path3):
    model2_state_dict, optimizer, loss = load_model(path2)
    model2.load_state_dict(model2_state_dict)
    load_network = True
    re_train = input("do you want to re-train the network? y/n")
    if re_train =="y":
       load_network = False
    elif re_train != "n":
       print("that was not a valid, I will proccees with the default, not training again")
    model = input("which model do you prefer? stable model (1) or experimental model (2)")
    #overwrite the default stable model
    if model == "2":
        model2 = ExperimentalTextClassifier(embeddings,padding_index = padding_token_id)
        model2_state_dict, optimizer, loss = load_model(path2)
        model2.load_state_dict(model2_state_dict)
    elif not model == "1":
        print("you did not give a valid number, I will continue with the default stable model")
